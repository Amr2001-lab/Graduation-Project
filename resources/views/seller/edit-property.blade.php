...
<body>
  ...
  <main class="container" style="padding: 40px 0;">
    <div class="form-card">
      ...
      {{-- ===== STAND-ALONE REMOVE TOUR FORM ===== --}}
      @if ($apartment->virtual_tour_path)
        <div class="form-group">
          <label>Current Virtual Tour</label>
          <iframe
            src="{{ asset($apartment->virtual_tour_path . '/index.html') }}"
            style="width:100%; height:400px; border:1px solid #ccc;"
            loading="lazy"
          ></iframe>

          <form action="{{ route('seller.properties.removeTour', $apartment->id) }}"
                method="POST"
                style="display:inline-block; margin-top:0.5rem;">
            @csrf
            @method('DELETE')
            <button type="submit" class="button danger">Remove Tour</button>
          </form>
        </div>
      @endif

      {{-- ===== MAIN UPDATE FORM ===== --}}
      <form action="{{ route('seller.properties.update', $apartment->id) }}"
            method="POST"
        ...
        {{-- Upload New Tour ZIP --}}
        <div class="form-group">
          <label for="tour_zip">Upload Virtual Tour (ZIP)</label>
          <input
            type="file"
            id="tour_zip"
            name="tour_zip"
          ...
          >
          <small>
            Must contain <code>index.html</code>, a <code>Build/</code> &amp; <code>TemplateData/</code> folder
          </small>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="current-images-container">
            @forelse ($apartment->images as $image)
              <div class="current-image-item" style="display:inline-block; margin:0.5rem; text-align:center;">
                <img
                  src="{{ asset('storage/Images/' . $image->image_url) }}"
                  style="max-width:150px; display:block; margin-bottom:0.5rem;"
                  alt="Image #{{ $loop->iteration }}"
                >
                <form action="{{ route('seller.images.destroy', $image->id) }}" method="POST">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="button danger btn-sm">Remove</button>
                </form>
              </div>
            @empty
        ...
        {{-- Submit --}}
        <button type="submit" class="nav-btn" style="margin-top:1.5rem;">
          Update Property
        </button>
      </form>
    </div>
